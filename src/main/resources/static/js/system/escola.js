...
$(document).ready(function () {
	...
	$("#setinepconfig").click(function(data){
		if ($("#inep").val() === ""){
			var inep = 0;
		}else{
			var inep = $("#inep").val();
		...
		graphql.request(uri, '{setConfig(conf: {escolaInep:' + inep + ', anoLetivo: 2019})}').then(function(data){
			if (data.setConfig === "OK"){
				...
				$("#escfields").prop("hidden", true);
				
				if ($("#inep").val() === ""){
					alert("Não pode ser vazio!");
				}else{
					alert("INEP inexistente");
				}
	...
	$("#uf").change(function(data){
		$("#ibge").empty();
		$("#ibge").append($('<option value="">Selecione o município</option>'));
		graphql.request(uri, '{filtrarMunicipioByUfId(ufid:' + data.target.value + '){id nome cod}}').then(function(data){
			$.each(data.filtrarMunicipioByUfId, function (i, mun) {
				$("#ibge").append($('<option value="' + mun.id + '" name="' + mun.cod + '">' + mun.nome + '</option>'));
